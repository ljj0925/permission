...
<mapper namespace="${content.dao.classPackage}.${content.dao.className}">
    ...
    <resultMap id="${content.mapper.item.resultMap.value}"
               type="${content.entity.classPackage}.${content.entity.className}">
        <#if content.entity.primaryKeyAttr??>
            <id column="${content.entity.primaryKeyAttr.columnName}"
                property="${content.entity.primaryKeyAttr.field}" /></#if>
    ...
    <select id="${content.dao.item.count.value}" parameterType="${content.assist.classPackage}.Assist"
    ...
    <select id="${content.dao.item.select.value}" parameterType="${content.assist.classPackage}.Assist"
            resultMap="${content.mapper.item.resultMap.value}">
        select * from
        (
        select
        ...
        ROW_NUMBER() over(
        <choose>
            <when test="order!=null">${r'${order}'}</when>
            <otherwise>order by ${content.entity.primaryKeyAttr.columnName}</otherwise>
        </choose>
        ) as page,
        <choose>
        ...
        <choose>
            <when test="startRow!=null">where page &gt; ${r'#{startRow}'}
                <if test="rowSize!=null">and page &lt;=
                    <if test="startRow!=null">${r'#{startRow}'}+</if>${r'#{rowSize}'} </if>
            </when>
            <otherwise>
                <if test="rowSize!=null">where page &lt;= ${r'#{rowSize}'}</if>
            </otherwise>
        </choose>
    </select>
    <#if content.entity.primaryKeyAttr??>
        <!-- 通过${content.entity.className}的id获得对应数据库中表的数据对象 -->
        <select id="${content.dao.item.selectById.value}" parameterType="${content.entity.primaryKeyAttr.javaType}"
                resultMap="${content.mapper.item.resultMap.value}">
            select
            <include refid="${content.entity.tableName}_Column"/>
            from ${content.entity.tableName}
    ...
    <select id="${content.dao.item.selectByObjSingle.value}"
            parameterType="${content.entity.classPackage}.${content.entity.className}"
            resultMap="${content.mapper.item.resultMap.value}">
        select top 1
        <include refid="${content.entity.tableName}_Column"/>
        from ${content.entity.tableName}
        <where>
            ...
            <#if content.entity.primaryKeyAttr??>
                <if test="${content.entity.primaryKeyAttr.field} != null">
                and ${content.entity.primaryKeyAttr.columnName}
                = ${r'#{'}${content.entity.primaryKeyAttr.field}${r'}'} </if></#if>
        </where>
        LIMIT 1
    </select>

    <!-- 获得${content.entity.className}对象,以参数${content.entity.className}对象中不为空的属性作为条件进行查询 -->
    <select id="${content.dao.item.selectByObj.value}"
            parameterType="${content.entity.classPackage}.${content.entity.className}"
            resultMap="${content.mapper.item.resultMap.value}">
        select
        <include refid="${content.entity.tableName}_Column"/>
    ...
    <insert id="${content.dao.item.insertAll.value}"
            parameterType="${content.entity.classPackage}.${content.entity.className}">
        insert into
    ...
    <insert id="${content.dao.item.insertNotNull.value}"
            parameterType="${content.entity.classPackage}.${content.entity.className}">
    ...
    <insert id="${content.dao.item.insertBatch.value}" parameterType="ArrayList">
        insert into ${content.entity.tableName}
        (<#list content.entity.attrs as item>${item.columnName}<#if item?has_next>,</#if></#list>) values
    ...
    <#if content.entity.primaryKeyAttr??>
        ...
        <update id="${content.dao.item.updateAllById.value}"
                parameterType="${content.entity.classPackage}.${content.entity.className}">
    ...
    <#if content.entity.primaryKeyAttr??>
        ...
        <update id="${content.dao.item.updateNotNullById.value}"
                parameterType="${content.entity.classPackage}.${content.entity.className}">
